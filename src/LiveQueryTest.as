package {
  ...
  public class LiveQueryTest extends TestCase {
    public var stateData:ArrayCollection = new ArrayCollection([
     {state: 'VA', cnt2: 2, cnt: 1},
     {state: 'PA', cnt2: 2, cnt: 2},
     {state: 'VA', cnt2: 2, cnt: 1},
     {state: 'PA', cnt2: 2, cnt: 2}
    ]);
    ...
    public function testLiveQuery():void {
      ...
      lq.query = select('state', {sum: sum('cnt')}).groupby('state');
      //new Query(['state', {sum: sum('cnt')}], null, ['state'], ['state']);
      ...
      stateData.addItem({state: 'VT', cnt2: 2, cnt: 5});
